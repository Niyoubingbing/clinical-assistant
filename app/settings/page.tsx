...
const THEME_OPTIONS = [
  { key: 'light', label: '??', icon: Sun },
  { key: 'dark', label: '??', icon: Moon },
  { key: 'system', label: '??', icon: Monitor }
] as const
...
export default function SettingsPage() {
  ...
  const [newQuickType, setNewQuickType] = useState('??')
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!confirm(`??? ${data.patients?.length || 0} ???? ${data.todos?.length || 0} ?????????????????`)) return
    ...
    <main className="min-h-screen pb-24 px-4 pt-4 bg-surface">
      ...
      <div className="space-y-6">
        <section className="bg-card rounded-xl border border-custom p-4">
          <h2 className="font-semibold text-main mb-3">??</h2>
        ...
        <section className="bg-card rounded-xl border border-custom p-4">
          <h2 className="font-semibold text-main mb-3">????</h2>
          <p className="text-xs text-muted mb-2">?????????????????</p>
          <textarea
            value={orderText}
          ...
          />
          <button onClick={handleSaveOrder} className="mt-3 w-full py-2.5 rounded-xl bg-primary text-white font-medium">????</button>
        </section>
