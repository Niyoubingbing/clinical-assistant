  ...
  if (!patient) {
    return (
      <main className="min-h-screen p-4 bg-surface flex flex-col items-center justify-center">
        <div className="text-muted text-sm mb-4">?????</div>
    ...
    <main className="min-h-screen pb-24 px-4 pt-4 bg-surface">
      ...
      <div className="bg-card rounded-xl border border-custom p-4 mb-4">
        ...
        <div className="grid grid-cols-2 gap-2 text-sm">
          ...
          <div className="text-muted">????</div>
          <div className="text-main text-right">{patient.dressingFrequency ? `${patient.dressingFrequency} ?` : '-'}</div>
          <div className="text-muted">????</div>
      ...
      <div className="space-y-3">
        ...
        {todos.length === 0 && <div className="text-center text-muted py-8 text-sm">????</div>}
      </div>

      <BottomSheet open={sheet === 'todo'} onClose={() => setSheet(null)} title="????">
        <TodoForm patientId={id} onSaved={() => { load(); setSheet(null) }} />
      </BottomSheet>
      <BottomSheet open={sheet === 'edit'} onClose={() => setSheet(null)} title="????">
